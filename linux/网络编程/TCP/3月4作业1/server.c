...
void *send_msgto_client1(void *arg)
{
	...
	{
		p = p->next;
		if (strcmp(p->ip, args->target_ip) == 0 && p->port == args->target_port)
		{
			bzero(buf, 2048);
			sprintf(buf, "%s@%s@%hu", args->msg, args->target_ip, args->target_port);
			send(p->sock, buf, strlen(buf), 0);

			printf("成功转发消息到 %s:%hu：%s\n", args->target_ip, args->target_port, args->msg);
			break;
		}
	}
...
void *recv_client_msg(void *arg)
{
	...
	while (1)
	{
		...
		char *tokens[4] = {NULL}; // 存储分割后的各段字符串
		int i = 0;
		tokens[i] = strtok(rbuf, "@"); // 第一次分割
		while (tokens[i] != NULL && i < 3)
		{ // 防止数组越界
			i++;
			tokens[i] = strtok(NULL, "@"); // 后续分割
		}
		// 检查解析结果是否完整
		if (tokens[0] && tokens[1] && tokens[2] && tokens[3] && strcmp(tokens[0], "msg") == 0)
		{
			// 匹配目标IP（192.168.11.150）
			if (strcmp(tokens[1], "192.168.11.150") == 0)
			{
				// 匹配目标端口（10000/10001/10002）
				unsigned short target_port = atoi(tokens[2]);
				if (target_port == 10000 || target_port == 10001 || target_port == 10002)
				{
					printf("消息是发送给客户端%d的！\n", target_port - 9999);

					// 封装转发参数（必须用堆内存，栈内存会失效）
					SendArgs *args = (SendArgs *)malloc(sizeof(SendArgs));
					strncpy(args->target_ip, tokens[1], sizeof(args->target_ip) - 1);
					args->target_port = target_port;
					strncpy(args->msg, tokens[3], sizeof(args->msg) - 1);

					// 创建转发线程（传递封装好的结构体指针）
					if (pthread_create(&otherid, NULL, send_msgto_client1, args) != 0)
					{
						perror("创建转发线程失败");
						free(args);
					}
