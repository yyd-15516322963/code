...
// 动态商品列表配置
#define MAX_GOODS_COUNT 50                             // 最大商品数
goods_item_t global_goods_list[MAX_GOODS_COUNT] = {0}; // 全局商品列表
int current_goods_count = 0;                           // 当前商品数量
...
void init_default_goods()
{
    ...
    global_goods_list[current_goods_count++] = g1;
    ...
    global_goods_list[current_goods_count++] = g2;
    ...
    global_goods_list[current_goods_count++] = g3;
    ...
    global_goods_list[current_goods_count++] = g4;
    ...
    global_goods_list[current_goods_count++] = g5;
    ...
    global_goods_list[current_goods_count++] = g6;
    ...
    global_goods_list[current_goods_count++] = g7;
    ...
    global_goods_list[current_goods_count++] = g8;
}

// 1. 新增商品（0成功/-1满/-2ID重复）
int add_goods(goods_item_t new_goods)
{
    if (current_goods_count >= MAX_GOODS_COUNT)
        return -1;
    // 检查ID重复
    for (int i = 0; i < current_goods_count; i++)
    {
        if (global_goods_list[i].id == new_goods.id)
            return -2;
    }
...
// 3. 修改商品（通过ID）
int update_goods(int goods_id, goods_item_t new_info)
{
    for (int i = 0; i < current_goods_count; i++)
    {
        if (global_goods_list[i].id == goods_id)
...
// 4. 查询商品（通过ID）
goods_item_t *query_goods(int goods_id)
{
    for (int i = 0; i < current_goods_count; i++)
    {
        if (global_goods_list[i].id == goods_id)
...
void add_to_cart_btn_cb(lv_event_t *e)
{
    ...
    if (ret != -1)
        lv_label_set_text(toast, tip_text);
...
void refresh_admin_goods_list()
{
    ...
    if (admin_goods_list_label == NULL)
        return;
...
void admin_update_goods_cb(lv_event_t *e)
{
    ...
    int id = atoi(id_str);
    goods_item_t *old_goods = query_goods(id);
    if (old_goods == NULL)
    {
        lv_obj_t *toast = lv_label_create(lv_scr_act());
        lv_label_set_text(toast, "Goods not found!");
    ...
    int ret = update_goods(id, new_info);

    lv_obj_t *toast = lv_label_create(lv_scr_act());
    if (ret == 0)
    {
        lv_label_set_text(toast, "Update success!");
        refresh_admin_goods_list();
        refresh_purchase_page();
        // 同步刷新购物车中的商品信息
        for (int i = 0; i < cart_item_count; i++)
        {
            if (shopping_cart[i].goods.id == id)
            {
                shopping_cart[i].goods = new_info;
                break;
            }
        }
        refresh_shopping_cart_page();
    ...
    {
        lv_label_set_text(toast, "Update failed!");
    }
    lv_obj_align(toast, LV_ALIGN_TOP_MID, 0, 20);
    lv_obj_set_style_text_font(toast, &lv_font_montserrat_16, LV_STATE_DEFAULT);
    lv_timer_create(toast_timer_cb, 2000, toast);
...
{
    if (tab2 == NULL || !lv_obj_is_valid(tab2) ||
        tab2_1 == NULL || !lv_obj_is_valid(tab2_1) ||
        tab2_2 == NULL || !lv_obj_is_valid(tab2_2) ||
        tab2_3 == NULL || !lv_obj_is_valid(tab2_3))
    {
    ...
    for (int i = 0; i < current_goods_count; i++)
    {
        goods_item_t *g = &global_goods_list[i];
        lv_obj_t *parent_tab = NULL;
        int idx = -1;

        // 按ID分类
        if (g->id >= 1001 && g->id <= 1005)
        { // 显示器
            if (monitor_idx >= max_item_per_tab)
                continue;
            parent_tab = tab2_1;
            idx = monitor_idx++;
        }
        else if (g->id >= 1006 && g->id <= 1010)
        { // 主机
            if (host_idx >= max_item_per_tab)
                continue;
            parent_tab = tab2_2;
            idx = host_idx++;
        }
